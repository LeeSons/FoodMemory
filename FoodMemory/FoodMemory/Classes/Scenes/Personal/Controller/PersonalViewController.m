...
- (void)viewDidLoad {
    ...
    self.menuArray = @[@"我去吃过",@"我想去吃",@"我收藏的DIY"];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (indexPath.row == 0) {
        ...
        cell.imageView.image = [UIImage imageNamed:@"iconfont-sc"];
    }
    return cell;
}
...
- (void)viewDidAppear:(BOOL)animated{
    ...
    if (self.currentUser == nil) {
        ...
        self.lblUnickName.text = [self.currentUser objectForKey:@"userNickName"];
        self.lblSign.text = [self.currentUser objectForKey:@"sign"];
        if ([[self.currentUser objectForKey:@"gender"] isEqualToString:@"F"]) {
